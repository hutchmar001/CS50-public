...
def search():
    ...
    if request.method == "POST":
        ...
        lst = db1.execute("SELECT * FROM verses WHERE text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? \
            ...
            OR text LIKE ? LIMIT 10 OFFSET 10;", ('% ' + search + ' %'), ('%' + search + ',%'), ('%"' + search + '%'), ('%' + search + '?%'), ('%' + search + '!%'), ('%\'' + search + '%'), ('%' + search + '.%'), ('%.' + search + '%'), ('%' + search + ']%'), ('%' + search + ';%'), ('%,' + search + '%'), ('%' + search + '-%'), ('%-' + search + '%'), ('%(' + search + ')%'))
...
def verse():
    ...
    if request.method == "POST":
        ...
        if bible_name and bible_chapter and bible_verse:
            ...
            return render_template('Bible.html', lst=lst, display1="visible", display2="none", display3="none", display_title="none", display_select="none", display_img="none", display_highlight="none", search_upper="", search_lower="")
