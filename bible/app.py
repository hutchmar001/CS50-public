...
def search():
    ...
    if request.method == "POST":
        ...
        # Bible
        s = db1.execute("SELECT * FROM verses WHERE text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ?", ('% ' + search + ' %'), ('%' + search + ' %'), ('% ' + search + '%'), ('%(' + search + ')%'))
        ...
        # Quran
        s = db2.execute("SELECT * FROM verses WHERE text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ?", ('% ' + search + ' %'), ('%' + search + ' %'), ('% ' + search + '%'), ('%(' + search + ')%'))
        ...
        # Bhagavad Gita
        s = db3.execute("SELECT * FROM verses WHERE text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ? OR text LIKE ?", ('% ' + search + ' %'), ('%' + search + ',%'), ('%"' + search + '%'), ('%' + search + '?%'), ('%"' + search + '!%'), ('%\'' + search + '%'), ('%' + search + '.%'), ('%(' + search + ')%'))
        print(s)
        if s:
            num = 1
            for i in s:
